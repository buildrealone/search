    ...
    if (req.method === "GET") { 
        ...
        for (let i = 0; i < categories.length; i += 1) { 
            ...
            const promises = categories.map(async (category) => {
                if (category === "datalab") {
                    const data = await useDataLab({ keyword, category, korea_month_ago, korea_yesterday });
                    return data;
                }
            ...
            return res.json({
                ...
                kvolumeItems: promiseResults[1]?.kvolume?.keywordList?.slice(0, 5),
                newsItems: promiseResults[2]?.news?.items,
                blogItems: promiseResults[3]?.blog?.items,
                cafeItems: promiseResults[4]?.cafearticle?.items,
                kinItems: promiseResults[5]?.kin?.items,
